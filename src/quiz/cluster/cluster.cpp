...
void get_nearby(const std::vector<std::vector<float>>& points, int idx, KdTree* tree, float distanceTol, std::vector<int>& visit, std::vector<int>& cluster)
{
    visit[idx] = 1; // visited
    cluster.push_back(idx);
    std::vector<int> near_points = tree->search(points[idx], distanceTol);
    for(auto np: near_points){
        if(!visit[np])
        {
            get_nearby(points, np, tree, distanceTol, visit, cluster);
        }
    }
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
    ...
    std::vector<int> visit(points.size(), 0);
    ...
    for(auto i = 0; i < points.size();i++)
    {
        if(!visit[i])
        {
            ...
            get_nearby(points, i, tree, distanceTol, visit, cluster);
