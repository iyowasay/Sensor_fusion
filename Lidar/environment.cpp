...
void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer, ProcessPointClouds<pcl::PointXYZI>* point_processor2, const pcl::PointCloud<pcl::PointXYZI>::Ptr& input_cloud)
{
    ...
    bool render_obstacle = false;// bool render_road = true;
    bool render_cluster = true;
    bool render_box =true;

    //ProcessPointClouds<pcl::PointXYZI>* point_processor2 = new ProcessPointClouds<pcl::PointXYZI>();
    ...
    float vox_resolution = 0.3f;
    // define the region you can see
    std::vector<float> minsize{-20, -6, -2.5};
    std::vector<float> maxsize{30, 7, 2.5};
    pcl::PointCloud<pcl::PointXYZI>::Ptr filtered_cloud = point_processor2->FilterCloud(input_cloud, vox_resolution, Eigen::Vector4f(minsize[0],minsize[1],minsize[2],1), Eigen::Vector4f(maxsize[0],maxsize[1],maxsize[2],1));
    if(renderFiltered)
